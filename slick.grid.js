...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function appendRowHtml(stringArray, row, range, dataLength) {
            ...
            //添加行样式计算
            var stylestr="";
            if(options.rowStyleExpression){
                if(!d.__groupTotals && !d.__group){
                   var ststr=options.rowStyleExpression;
                    for (var i in d) {
                        eval("var " + i + " = \'" + d[i] + "\'");
                    }
                    var styleObj = eval(ststr);
                    stylestr=JSON.stringify(styleObj);
                    stylestr= stylestr.replace("null","");
                    stylestr= stylestr.replace(/\}|{|\"/g,"");
                    stylestr=stylestr.replace("_","-");
                }
            }
            ...
            stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + getRowTop(row) + "px; "+stylestr+"'>");
            var colspan, m;
            for (var i = 0, ii = columns.length; i < ii; i++) {
                m = columns[i];
                colspan = 1;
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            var m = columns[cell] = $.extend({}, columnDefaults, columns[cell]);
            ...
            //这里改变单元格的样式(对齐方式align,color,),添加cell点击事件,样式运算

            var cellclick = "", celldbclick = "", styleex = "", stylestr=" ";
            if(m.align){
                styleex=styleex+" text-align:"+ m.align+";";
            }
            if(m.color){
                styleex=styleex+" color:"+ m.color+";";
            }
            if(m.backgroundColor){
                styleex=styleex+" background-color:"+ m.backgroundColor+";";
            }
            if (m.styleExpression) {

               if(!item.__groupTotals && !item.__group){
                   var exstr = m.styleExpression;
                   for (var i in item) {
                       eval("var " + i + " = \'" + item[i] + "\'");
                   }
                   var styleObj = eval(exstr);
                   stylestr=JSON.stringify(styleObj);
                   stylestr= stylestr.replace("null","");
                   stylestr= stylestr.replace(/\}|{|\"/g,"");
                   stylestr=stylestr.replace("_","-");
                   styleex= styleex+stylestr;
               }
            }
            stringArray.push("<div class='" + cellCss + "' " +
                " style='background-color: transparent;"+styleex+"'  >");
        ...
        function exportDefaultGridDataAll(options) {
            ...
            $input.appendTo("body");
            if (grid.getOptions().pagingOptions) {
                toQuery("_gridHead_," + grid.getOptions().pagingOptions.submitIds, Base.globvar.contextPath + "/exportGridDataAllExcel.do", options);
            } else {
                toQuery("_gridHead_", Base.globvar.contextPath + "/exportGridDataAllExcel.do", options);
            }
            $input.remove();
        }
